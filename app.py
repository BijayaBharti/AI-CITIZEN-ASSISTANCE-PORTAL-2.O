...
def render_hero_section():
    ...
    with hc1:
        st.button(f"🚀 {t('cta_explore')}", key="hero_cta_explore", use_container_width=True)
    with hc2:
        st.button(f"💬 {t('cta_start')}", key="hero_cta_start", use_container_width=True)
...
def render_stats_dashboard():
    ...
    agents_count = len(TAB_NAMES) if "TAB_NAMES" in globals() else 9
...
#=========================================================== 
# STEP 4: SIDEBAR - BRANDING, LANGUAGE, THEME & API CONFIG
#===========================================================
...
with st.sidebar:
    ...
    # ---- Decorative navigation (actual routing stays on the tabs below) ----
    st.markdown(f"**{t('nav_header')}**")
    nav_items = [
        ("🏠", t("nav_home")), ("💬", t("nav_assistant")), ("🏛️", t("nav_scheme")),
        ("📤", t("nav_upload")), ("📝", t("nav_complaint")), ("✅", t("nav_checklist")),
        ("📰", t("nav_updates")), ("⚙️", t("nav_settings")),
    ]
    for icon, label in nav_items:
        st.markdown(f'<div class="citizenai-nav-item">{icon} &nbsp; {label}</div>', unsafe_allow_html=True)

    st.markdown("---")

    # ---- API CONFIG (unchanged logic — same keys/behavior as before) ----
    st.subheader("⚙️ SET API CONFIG")

    GOOGLE_API_KEY = st.text_input(
        "GOOGLE_API_KEY", type="password", value=os.getenv("GOOGLE_API_KEY", "")
    )
    TAVILY_API_KEY = st.text_input(
        "TAVILY_API_KEY (optional, for Live Search)", type="password", value=os.getenv("TAVILY_API_KEY", "")
    )
    ...
    st.caption(
        ...
        "Get a free Tavily API key from tavily.com for the Live Search feature."
    )

    if not GOOGLE_API_KEY:
        st.warning("Enter your GOOGLE_API_KEY to enable all AI features.")
...
#=========================================================== 
# STEP 5: HERO SECTION + LIVE STATS DASHBOARD
#===========================================================
render_hero_section()
...
def require_google_key():
    if not GOOGLE_API_KEY:
        st.error("Please enter your GOOGLE_API_KEY in the sidebar to use this feature.")
        return False
    return True
...
# ===========================================================
    ...
    def dashboard_card(title, value, subtitle, icon, color):
        # ===========================================================
        # FEATURE CARD
        # ===========================================================
...
if selected_page == "🔎 Live Search":
    ...
    if not TAVILY_IMPORT_OK:
        ...
        st.info("Enter your TAVILY_API_KEY in the sidebar to enable live search.")
