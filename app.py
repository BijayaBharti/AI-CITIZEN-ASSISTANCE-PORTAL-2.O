...
def t(key):
    ...
    return table.get(key, UI_LABELS["English"].get(key, key))
...
with st.sidebar:
    st.markdown(
        '<p class="citizenai-sidebar-logo">🏛️ CitizenAI</p>'
        '<p class="citizenai-sidebar-sub">AI Powered Government Assistant</p>',
        unsafe_allow_html=True,
    )
    ...
    with st.expander("🔑 API Keys", expanded=not bool(st.session_state.google_api_key)):
        ...
        if not st.session_state.google_api_key:
            st.warning("Add your GOOGLE_API_KEY to enable AI features.")
        else:
    ...
    with st.container(border=True):
        ...
        st.caption("We are here to help you!")
        if st.button("💬 Chat with Assistant", key="need_help_btn", use_container_width=True):
...
def get_llm(api_key, model="gemini-2.5-flash", temperature=0.3):
...
def transcribe_voice_input(audio_bytes, language="English"):
    ...
    try:
        ...
        speech_lang = "hi-IN" if language in ("Hindi", "Hinglish", "Marathi") else "en-IN"
        return recognizer.recognize_google(audio_data, language=speech_lang)
    except Exception as e:
        st.warning(f"Could not understand the audio: {e}")
        return None
...
def render_top_header():
    ...
    with h2:
        st.session_state.search_query = st.text_input(
            ...
            placeholder="🔍 Search anything...", label_visibility="collapsed",
        )
    with h3:
...
def render_home_page():
    if not GOOGLE_API_KEY:
        ...
        with w1:
            st.warning("⚠️ Add your Google API key on the Settings page to activate every AI feature.")
        with w2:
            if st.button("Go to Settings", key="home_go_settings", use_container_width=True):
                st.session_state.active_page = "settings"
                st.rerun()
    ...
    <div class="citizenai-hero">
        <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap;">
            <div style="max-width:70%;">
                <div style="font-size:1.05rem;">👋 Namaste!</div>
                <h1 style="margin-top:2px;">Welcome to <span style="color:#FFE1B0;">AI Citizen</span> Assistance Portal</h1>
                <p>Your one-stop solution for all government services and information.</p>
                <div class="citizenai-badge-row">
                    <span class="citizenai-badge">✨ Smart AI</span>
                    <span class="citizenai-badge">✅ Accurate Info</span>
                    <span class="citizenai-badge">🔒 Secure</span>
                    <span class="citizenai-badge">🕘 24/7 Available</span>
                </div>
            </div>
    ...
    with f1:
        ...
        st.caption("Powerful tools to help you")
    with f2:
        if st.button("⚙️ Customize", key="customize_dashboard_btn", use_container_width=True):
            st.session_state.active_page = "settings"
            st.rerun()
        ...
        for col, page in zip(cols, row_pages):
            with col:
                with st.container(border=True):
                    ...
                    """, unsafe_allow_html=True)
                    if st.button("Open →", key=f"grid_{page['key']}", use_container_width=True):
                        st.session_state.active_page = page["key"]
                        st.rerun()
    ...
    st.markdown("#### 📰 Latest Sarkari Updates")
    if TAVILY_IMPORT_OK and TAVILY_API_KEY:
        try:
            ...
            if updates:
                ...
                st.caption("No live updates found right now — try again later.")
        except Exception as e:
            st.caption(f"Couldn't load live updates: {e}")
    else:
        st.info(
            "📡 Add your TAVILY_API_KEY on the Settings page to show real-time government scheme news here."
        )
...
def render_page_assistant():
    ...
    with st.expander("🎤 Voice Input (Beta)"):
        if not VOICE_INPUT_AVAILABLE:
            st.info(
                "Voice input needs two extra packages: `streamlit-mic-recorder` and `SpeechRecognition`. "
                "Install them (see requirements.txt) to enable this. Typing still works as usual above."
            )
        else:
            st.caption("Tap the mic, speak your question, then tap again to stop.")
            voice_audio = mic_recorder(start_prompt="🎤 Start", stop_prompt="⏹ Stop", key="voice_recorder")
            if voice_audio and voice_audio.get("bytes"):
                transcribed = transcribe_voice_input(voice_audio["bytes"], st.session_state.ui_language)
                if transcribed:
                    st.success(f"Recognized: \"{transcribed}\"")
                    if st.button("📤 Send this as my question", key="send_voice_msg"):
                        process_assistant_message(transcribed)

    user_msg = st.chat_input("Ask a question about government services...")
    if user_msg:
...
def render_page_ocr():
    ...
    if not EASYOCR_IMPORT_OK:
        st.warning("`easyocr` is not installed. Image OCR is disabled, but PDF text extraction still works. "
                    "Run `pip install easyocr` to enable image OCR.")

    ocr_file = st.file_uploader(
        "Upload a document (jpg, jpeg, png, pdf)", type=["jpg", "jpeg", "png", "pdf"], key="ocr_uploader"
    )

    if ocr_file is not None:
        if st.button("🔍 Extract & Analyze Document"):
            with st.spinner("🔍 Analyzing document... Checking document type..."):
                try:
    ...
    if st.session_state.ocr_text:
        st.success(f"Detected Document Type: **{st.session_state.ocr_doc_type}**")
...
def render_page_scheme():
    ...
    with st.form("scheme_form"):
        ...
        with col1:
            ...
            annual_income = st.number_input("Annual Family Income (₹)", min_value=0, value=150000, step=10000)
        with col2:
            state = st.selectbox(
                "State",
        ...
        submitted = st.form_submit_button("🔍 Find Matching Schemes")
    ...
    if submitted:
        ...
        if GOOGLE_API_KEY and matches:
            with st.spinner("🏛️ Checking Eligibility... Generating personalized advice..."):
    ...
    if st.session_state.scheme_matches is not None:
        ...
        if not matches:
            st.info("No matching schemes found for the given profile. Try adjusting the categories or income.")
        else:
            st.success(f"Found {len(matches)} matching scheme(s).")
            ...
            for scheme in matches:
                with st.expander(f"🏛️ {scheme['name']}"):
                    ...
                    st.write(f"**Official Link:** {scheme['official_link']}")
...
def render_page_rag():
    ...
    rag_file = st.file_uploader("Upload PDF File", type=["pdf"], key="rag_uploader")

    k_slider = st.slider("Select top-k value (chunks retrieved per question)", min_value=1, max_value=10, value=4)

    if rag_file is not None:
        if st.button("📥 Build Knowledge Base from PDF"):
            if require_google_key():
                with st.spinner("📚 Searching Government Database... Indexing document..."):
                    try:
    ...
    if st.session_state.vectorstore is not None:
        st.info(f"📄 Active document: **{st.session_state.rag_pdf_name}**")
        ...
        rag_question = st.chat_input("Ask a question about the uploaded document...")
        if rag_question:
            ...
            with st.chat_message("assistant"):
                if require_google_key():
                    with st.spinner("🔎 Checking Eligibility... Searching the document..."):
                        try:
        ...
        if st.button("🗑️ Clear RAG Chat"):
            st.session_state.rag_messages = []
            st.rerun()
    else:
        st.info("Upload a PDF and click 'Build Knowledge Base' to start chatting with your document.")
...
def render_page_search():
    ...
    if not TAVILY_IMPORT_OK:
        st.warning("`langchain-community`'s Tavily tool is not available. Run "
                    "`pip install tavily-python langchain-community` to enable this feature.")
    elif not TAVILY_API_KEY:
        st.info("Enter your TAVILY_API_KEY on the ⚙️ Settings page to enable live search.")

    search_query = st.text_input("Search the web", placeholder="e.g. latest PM-KISAN installment date 2026")
            ...
            with st.spinner("🔎 Searching Government Database..."):
                try:
                    ...
                    if GOOGLE_API_KEY and results:
                        with st.spinner("📝 Generating Response... Summarizing results..."):
...
def render_page_complaint():
    ...
    with st.form("complaint_form"):
        ...
        with c1:
            ...
            department = st.text_input("Department / Authority", placeholder="e.g. Municipal Corporation")
        with c2:
        ...
        complaint_submit = st.form_submit_button("✍️ Generate Complaint Letter")

    if complaint_submit:
        if not (full_name and description and subject):
            st.error("Please fill in at least your name, subject, and description.")
        else:
            prompt_text = f"""
...
"""
            with st.spinner("📝 Generating Response... Drafting your complaint letter..."):
    ...
    if st.session_state.complaint_text:
        st.text_area("Generated Complaint Letter", st.session_state.complaint_text, height=350)
...
def render_page_checklist():
    ...
    service = st.selectbox(
        "Select the service you need a checklist for",
    ...
    custom_service = ""
    if service == "Other":
        custom_service = st.text_input("Specify the service")
    ...
    if st.button("✅ Generate Checklist"):
        ...
        with st.spinner("✅ Analyzing... Generating your checklist..."):
...
def render_page_translator():
    ...
    t1, t2 = st.columns(2)
    with t1:
        source_text = st.text_area("Enter text to translate", height=180)
    with t2:
        target_lang = st.selectbox("Translate to", ["English", "Hindi", "Hinglish (Roman Hindi)"])
    ...
    if st.session_state.translation_text:
        st.text_area("Translated Text", st.session_state.translation_text, height=180)
...
def render_page_history():
    ...
    if not st.session_state.activity_log:
        st.info("No activity yet. Use any feature above and it will show up here.")
    else:
...
def render_page_settings():
    ...
    st.caption("Manage your profile and API keys. Theme and language stay in the sidebar for quick access.")

    st.markdown("##### 🧑 Your Profile")
    st.session_state.citizen_name = st.text_input("Display Name", key="citizen_name")
    ...
    st.caption(
        "Get a free Google Gemini API key from Google AI Studio (aistudio.google.com/app/apikey). "
        "Get a free Tavily API key from tavily.com for live search and the Home page's Latest Updates."
    )
    if not st.session_state.google_api_key:
        st.warning("Add your GOOGLE_API_KEY above to enable every AI feature.")
    else:
        st.success("Google API key is set.")
